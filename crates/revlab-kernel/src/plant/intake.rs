...
impl IntakeManifold {
    ...
    pub fn new(volume: f64, t_up: Port, m_dot_eng: Port, m_comp_in: Port,
               p_out: Port, t_out: Port, m_dot_in_out: Port,
               p_init: f64, t_init: f64) -> Self {
        IntakeManifold {
            p: p_init, t: t_init, volume,
            t_up, m_dot_eng, m_comp_in, p_out, t_out, m_dot_in_out,
...
impl Component for IntakeManifold {
    ...
    fn step(&mut self, _t: u16, ctx: &mut Ctx<'_>) {
        ...
        let m_in = ctx.bus.get(self.m_comp_in);
